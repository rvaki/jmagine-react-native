...
export default class mapScene extends Component {
	...
	componentWillMount()
  {
	...
	getPois = () =>
	{
		...
		fetch("http://jmagine.tokidev.fr/api/pois/get_all")
		.then((response) => response.text())
		.then((responseData) => {
			const parser = new DOMParser();
			const xmlDoc = parser.parseFromString(responseData);
      console.log('nbpoi : '+xmlDoc.getElementsByTagName("poi").length);
      //console.log('titre poi 2  : '+xmlDoc.getElementsByTagName("title")[1].childNodes[0].nodeValue);
      this.setState({isLoading : false});
      for(var i = 0 ; i <xmlDoc.getElementsByTagName("poi").length;i++)
      {
        this.setState({
  				pois: this.state.pois.concat([new poiC(
  					i,
  					xmlDoc.getElementsByTagName("title")[i].childNodes[0].nodeValue,
  					xmlDoc.getElementsByTagName("backgroundPic")[i].childNodes[0].nodeValue,
  					xmlDoc.getElementsByTagName("address")[i].childNodes[0].nodeValue,
  					xmlDoc.getElementsByTagName("lat")[i].childNodes[0].nodeValue,
  					xmlDoc.getElementsByTagName("lng")[i].childNodes[0].nodeValue,
  					xmlDoc.getElementsByTagName("next_poi")[i].childNodes[0].nodeValue,
  				)]),
  			});
      }
		})
